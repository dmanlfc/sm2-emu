...
//
// Ported from MAME's src/devices/sound/scspdsp.cpp (BSD-3-Clause,
// copyright-holders ElSemi, R. Belmont).
//
// Changes from upstream:
//   address_space *space              ScspMemory *memory
//   space->read_word / write_word     memory->scsp_read_word / scsp_write_word
//   util::sext(v, n)                  sign_extend(v, n)
//   SCSPDSP                           ScspDsp
//   BIT(v, n)                         a local macro of the same meaning
// The instruction loop is otherwise upstream's, so a fix there stays diffable.
...
void ScspDsp::Init()
{
	// Upstream clears the whole struct here, which in MAME is safe because the
	// address_space pointer is assigned afterwards. Ours is assigned at
	// construction and has to survive a reset, so it is saved across the clear.
	ScspMemory *const saved_memory = memory;
...
#undef DUMP
		...
		if (!ZERO)
		{
			if (BSEL)
				...
				B = sign_extend(TEMP[(TRA + DEC) & 0x7f], 24);
			if (NEGB)
				B = 0 - B;
		}
		else
		...
		if (XSEL)
			...
			X = sign_extend(TEMP[(TRA + DEC) & 0x7f], 24);
		...
		if (SHIFT == 0)
			...
			SHIFTED = sign_extend(ACC * 2, 24);
		else if (SHIFT == 3)
			SHIFTED = sign_extend(ACC, 24);
		...
		//if (0)
		{
			...
			{
				if (NOFL)
					MEMVAL = memory->scsp_read_word(ADDR) << 8;
				else
					MEMVAL = UNPACK(memory->scsp_read_word(ADDR));
			}
			if (MWT && (step & 1))
			{
				if (NOFL)
					memory->scsp_write_word(ADDR, SHIFTED >> 8);
				else
					memory->scsp_write_word(ADDR, PACK(SHIFTED));
			}
		}
