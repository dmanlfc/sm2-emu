...
void CoproSharc::control_write(u32 value)
{
    if (((value ^ m_control) & 0x80000000u) != 0) {
        if ((value & 0x80000000u) != 0) {
            ...
            // Set initial flag state after boot.
...
u64 CoproSharc::pm_read48(u32 address)
{
    if (address < kProgramWords) {
...
{
    if (address < kProgramWords) {
        m_program[address] = data & 0xffffffffffff;
    }
}

u32 CoproSharc::pm_read32(u32 address)
{
    // Upper 32 bits of the 48-bit word.
    if (address < kProgramWords) {
        return static_cast<u32>(m_program[address] >> 16);
    }
    return 0;
}
...
{
    if (address < kProgramWords) {
        // Write to upper 32 bits, preserve lower 16.
        m_program[address] = (static_cast<u64>(data) << 16) | (m_program[address] & 0xffff);
    }
}
...
void Model2B::register_write(u32 address, u32 value, u32 width)
{
    ...
    // SHARC IOP register write at 0x008c0000.
    if (address >= kSharcIop && address < kSharcIop + 0x1000) {
        m_copro.iop_write(address - kSharcIop, value);
        return;
    }
