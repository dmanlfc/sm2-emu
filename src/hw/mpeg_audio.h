...
//
// Ported verbatim from MAME's src/devices/sound/mpeg_audio.{h,cpp}
// (BSD-3-Clause, copyright-holders Olivier Galibert). The only change is the
// removal of MAME's "emu.h" umbrella include in favour of the standard headers
// the code actually uses, and wrapping the class in the project namespace. The
// decoder is self-contained -- plain arrays and floating-point math, no MAME
// framework -- which is why it ports as a drop-in.
//
// Used by the Sega Z80 Digital Sound Board (hw/dsbz80), which streams a game's
// background music as MPEG audio from a dedicated ROM.
#pragma once
